...
use crate::folder_manager::{FolderLayout,FolderStatus};
// ─────────────────────────────────────────────────────────────────────────────
// HEALTH
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// EXPORT COLLECTION
// Now delegates directly to zipops::export_collection which owns the Json
// extractor, matching the updated zipops signature.
// ─────────────────────────────────────────────────────────────────────────────
...
pub async fn export_collection(
    ...
    tokio::task::spawn_blocking(move || {
        zipops::zip_collection(&source, &output)
    })
    .await 
...
// ─────────────────────────────────────────────────────────────────────────────
// IMPORT ZIP
// Delegates directly — zipops::import_zip owns its own Json extractor.
// ─────────────────────────────────────────────────────────────────────────────
#[derive(Deserialize)]
...
pub async fn import_zip(
   ...
   let zip  = PathBuf::from(payload.zip);
    ...
    zipops::import_zip_impl(&zip, &dest)            // ← calls the plain sync helper
        .map_err(|e| e.to_string())?;
...
// ─────────────────────────────────────────────────────────────────────────────
// EXPORT BOOKMARKS
// Delegates directly — zipops::export_bookmarks owns its own Json extractor.
// ─────────────────────────────────────────────────────────────────────────────
...
pub async fn export_bookmarks(
    ...
    let output = PathBuf::from(payload.output);
    zipops::create_zip_from_bookmarks(&repo, &payload.eids, &output)  // ← plain sync helper
        .map_err(|e| e.to_string())?;
...
pub async fn create_static(
    ...
    zipops::create_static_website(&source, &output)
        .map_err(|e| e.to_string())?;
...
pub async fn export_static(
    ...
    zipops::export_static_website(&source, &output)
        .map_err(|e| e.to_string())?;
...
pub async fn generate_markdown(
    ...
    create_markdown(&repo, payload.per_file, records)
        .map_err(|e| e.to_string())?;
...
pub async fn generate_meta(
    ...
    create_markdown_meta(repo, payload.eids.into_iter())
        .map_err(|e| e.to_string())?;
...
/// GET /system/status
///
/// Inspects the edms folder structure and reports its health.
/// Never creates, deletes, or moves anything — purely diagnostic.
/// Use this as a Docker/k8s readiness probe or a pre-flight check
/// before calling other endpoints.
///
/// Response 200 — JSON:
/// {
///   "success": true,
///   "action": "already_healthy" | "folders_created" | "structure_reset",
///   "missing_folders": [],
///   "root_path": "/app/edms_root",
///   "message": "Folder structure is healthy. Server is ready."
/// }
pub async fn system_status(
    ...
    // Filesystem work runs on a blocking thread — never block the Tokio runtime
    let report = tokio::task::spawn_blocking(move || {
        folder_manager::verify_and_init(&root)
    })
    .await
    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
/// POST /system/init
///
/// Verifies the edms folder structure and repairs it automatically:
///   - Missing folders → created
///   - Corrupted structure → fully reset
///   - Healthy → no changes, still returns a success report
///
/// This replaces the old interactive yes/no prompt that broke Docker.
/// Call this endpoint once at startup (or from a Docker HEALTHCHECK)
/// before making any other requests.
///
/// Request body (optional):
///   {}                                    ← use default root path
///   { "root_path": "/app/edms_root" }    ← override for Docker
///
/// Response 200 — JSON:
/// {
///   "success": true,
///   "action": "folders_created",
///   "missing_folders": ["repo", "session-backup", "temp"],
///   "root_path": "/app/edms_root",
///   "message": "3 missing folder(s) were created. Server is ready."
/// }
///
/// Response 400 — if root_path is not absolute
/// Response 500 — if filesystem operations fail
pub async fn system_init(
    payload: Option<Json<SystemInitRequest>>,
) -> Result<Json<folder_manager::SystemInitReport>, (StatusCode, String)> {

    // Resolve root path: caller-supplied → default
        ...
        Some(custom) => {
            ...
            if !p.is_absolute() {
                return Err((
                    ...
                    format!("root_path must be an absolute path, got: '{}'", custom),
                ));
            }
            p
        }
